...
| **Listing visibility** | Only `status = 'active'` businesses are returned by search, SEO lists, lead routing, AND direct profile pages (`getBusinessBySlug`) — an inactive/pending/closed listing is never surfaced anywhere public. | `db/migrations/0001,0003`, `lib/queries.ts`, `lib/leads.ts` |
| **Bulk import integrity** | The importer validates the entire dataset against a `.strict()` zod schema (unknown keys rejected, `website` http(s)-only, country/slug patterns), upserts idempotently on `(source, externalId)` with a deterministic slug, and is fully parameterized. Run as an operator CLI (not a public endpoint). | `lib/import/*`, `scripts/import.ts` |
